...
void initial_partitioning::perform_initial_partitioning(const PartitionConfig & config, graph_access &  G) {
        ...
        if(config.graph_already_partitioned && !config.omit_given_partitioning) {
        ...
        if(!((config.graph_already_partitioned && config.no_new_initial_partitioning) || config.omit_given_partitioning)) {
        ...
        if(!(config.graph_already_partitioned && config.no_new_initial_partitioning)) {
