...
double quality_metrics::z_value(graph_access & G) {
        EdgeWeight imbalance = 0;
        forall_nodes(G, n) { 
                ...
                forall_out_edges(G, e, n) {
                        ...
                        if ( G.getEdgeWeight(e) > 0 && partitionIDSource != partitionIDTarget) {
                                imbalance += G.getEdgeWeight(e);
                        }
                        if ( G.getEdgeWeight(e) < 0 && partitionIDSource == partitionIDTarget) {
                                imbalance -= G.getEdgeWeight(e);
                        }
	...
	imbalance /= 2;
        return imbalance/(double)-neg_edges(G);
}

EdgeWeight quality_metrics::neg_edges(graph_access & G) {
        EdgeWeight NegativeEdgeWeight = 0;
        forall_nodes(G, n) { 
                forall_out_edges(G, e, n) {
			if (G.getEdgeWeight(e) < 0)
				NegativeEdgeWeight += G.getEdgeWeight(e);
                } endfor 
        } endfor
        return NegativeEdgeWeight/2;
}
...
double quality_metrics::error_rate(graph_access & G) {
    int count = 0;
    forall_nodes(G, node) {
        forall_out_edges(G, e, node) {
            NodeID target = G.getEdgeTarget(e);
            if(G.getPartitionIndex(node) == G.getPartitionIndex(target) && G.getEdgeWeight(e) < 0) {
                count += 1;
            } else if(G.getPartitionIndex(node) != G.getPartitionIndex(target) && G.getEdgeWeight(e) > 0) {
                count += 1;
            }
        } endfor
    } endfor
    return (double) count / G.number_of_edges();
}
...
double quality_metrics::balance(graph_access& G) {
        std::vector<PartitionID> part_weights(G.number_of_nodes(), 0);
