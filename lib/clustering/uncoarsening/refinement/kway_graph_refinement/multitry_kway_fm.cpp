...
//int multitry_kway_fm::start_more_locallized_search(PartitionConfig & config, graph_access & G, 
        ...
        //while(!todolist.empty()) {
                ...
                //if(moved_idx.find(node) == moved_idx.end() && maxgainer != INVALID_PARTITION) { // is boundary node? // before signed clustering: ext_degree > 0
                        ...
                        //if(init_neighbors) {
                                //forall_out_edges(G, e, node) {
                                        ...
                                        //if(moved_idx.find(target) == moved_idx.end()) {
                                                ...
                                                //if(maxgainer != INVALID_PARTITION) { // is boundary node? // before signed clustering: ext_degree > 0
